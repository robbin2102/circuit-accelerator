...
>
> Last updated: 2026-08-14 by Robbin — team plan re-sized from 6 to 5 hires against real Singapore
> salary research; use-of-funds table (§2) rebuilt bottom-up.
...
- **Runway this buys:** ~12 months, blended. This is directly derived from the use-of-funds table in §2, which is now built bottom-up from a real Singapore hiring plan, not a top-down guess: $740,780 total planned spend (team + inference + marketing, §2) over ~12 months = **~$61.7K/mo average burn**, leaving a ~$9,220 (1.2%) buffer against the $750,000 raise. Current pre-raise solo-founder burn is $5K/mo. **What's still unconfirmed is pacing, not the total:** exact offer dates and whether the 5 hires ramp in over 2–3 months as planned (§2) — that shifts effective monthly burn at the same total spend. Update this line once offers are being drafted.
...
<!-- 2026-08-14: replaced the earlier top-down 70/15/15 split with a bottom-up build. Team costs
     were priced against real Singapore market comp for 4-5yr-experience hires (NodeFlair,
     JobStreet, BGC Group, Apple SG, JustCo/WeWork listings, sourced 2026-08-14), not assumed —
     see calculation below the table. Original 6-hire plan (2 blockchain, 2 full-stack, 1 UI/UX,
     1 marketing) priced out to ~$697K-713K, 88-95% of the raise, and didn't fit alongside 15%
     inference + 15% marketing. Cut to 5 hires (dropped 1 blockchain dev, 1 full-stack dev) per
     founder decision — the reduced team fits with a small buffer. -->

| Spend | $ | % | Milestone it buys | Proof it produces | Target date | Linear project |
|---|---|---|---|---|---|---|
| Team — 1 Sr Blockchain Dev, 2 AI Engineers, 1 Full-Stack Dev, 1 Marketing Manager (5 hires, Singapore-based) + office, hardware, opex | $515,780 | 68.8% | Build Quant Terminal + Agent Vault infrastructure on top of the already-shipping Quant Agent | Live Terminal beta + usage dashboard; Agent Vaults open to top-5% traders | 2027-02 *(draft — confirm hiring timeline)* | → link |
| AI inference — GPT/Claude frontier model API costs | $112,500 | 15% | Agent runs live for beta cohort at scale | Uptime + inference-cost dashboard | Ongoing from beta launch | → link |
| User acquisition & marketing spend (ads, campaigns — distinct from the Marketing Manager's salary, above) | $112,500 | 15% | User acquisition toward the 10,000-trader critical mass Agent Vaults requires (per canon.md origin story) | 5,000 Terminal beta users signed up | 2027-02 *(draft)* | → link |
...
**Team cost build-out (sourced 2026-08-14):** base salaries at Singapore market mid-point for 4-5yr experience, converted to USD — Sr Blockchain Dev $84,360, AI Engineer $88,800 ×2, Full-Stack Dev $71,040, Marketing Manager $66,600 = $399,600 base. Employer overhead ×1.15 (blended, assumes Employment Pass / foreign hires — CPF-exempt, unlike citizen/PR hires which would run ×1.20-1.30) = $459,540. Plus office (small team) $31,080/yr, one-time hardware (5 laptops) $11,840, other opex (corp secretary, accounting, insurance, core SaaS) $13,320/yr. Total **$515,780 USD** — the full-run-rate annual figure. The underlying market comp was researched in Singapore-dollar terms (NodeFlair, JobStreet, BGC Group, Apple SG, JustCo/WeWork), converted once to USD; the model tracks USD only from here on, per the currency rule (canon.md). The actual 2-3 month hiring ramp (§1) means real Year-1 cash spend on team lands lower, closer to $487,700.
...
**On team risk:** the hiring plan (1 Sr Blockchain Dev, 2 AI Engineers, 1 Full-Stack Dev, 1 Marketing Manager — 5 hires, Singapore-based) is a genuine bandwidth constraint for one founder — sourcing blockchain-specific technical talent without a co-founder to parallelize recruiting is real, independent of founder quality. That's distinct from "solo founder" as a generic red flag: the founder's operating history (scaled an edtech venture to $1M ARR, shipped deed.so with $3M in volume) is real evidence against the generic version of that concern. The specific, narrower risk is hiring execution speed, not founder capability. **A second-order risk worth naming directly:** the team was originally sized at 6 hires (2 blockchain devs, 2 full-stack devs, 1 UI/UX, 1 marketing) before bottom-up Singapore salary research showed that plan consumed 88-95% of the raise — it was cut to 5 hires (§2) to fit the budget, not because 5 was the engineering-determined right size. Watch this: if the Terminal build genuinely needs a second blockchain or full-stack hire, the current plan doesn't have room for one without either a bigger raise or a later hire funded by early revenue.
